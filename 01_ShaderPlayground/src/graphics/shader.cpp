...
void Shader::ChangeFragment(const std::string& newFsPath) {
    glDetachShader(m_id, m_fragment);

    std::string fsContent = readFile(newFsPath);

    if (fsContent.empty()) return;

    const char* fsSource = fsContent.c_str();

    m_fragment = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(m_fragment, 1, &fsSource, nullptr);
    glCompileShader(m_fragment);

    if (!verifyShader(m_fragment, false)) {
